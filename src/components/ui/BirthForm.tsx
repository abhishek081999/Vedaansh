...
export function BirthForm({ onResult, onLoading, autoSubmit = false, initialName = 'Transit' }: BirthFormProps) {
  ...
  const jhd_inputRef = useRef<HTMLInputElement>(null)
  const [jhdError, setJhdError] = useState<string | null>(null)
  ...
  function parseJHD(text: string): void {
    setJhdError(null)
    try {
      ...
      // Name
      const parsedName = lines['name'] || lines['fullname'] || ''
      if (parsedName) setName(parsedName)

      // Date: JHD uses MM/DD/YYYY or YYYY/MM/DD or DD/MM/YYYY
      const rawDate = lines['date'] || lines['birthdate'] || ''
      if (rawDate) {
      ...
      const rawTime = lines['time'] || lines['birthtime'] || ''
      if (rawTime) {
        const t = rawTime.replace(/[ap]m$/i,'').trim()
      ...
      // Latitude
      const rawLat = lines['lat'] || lines['latitude'] || ''
      if (rawLat) setLat(parseFloat(rawLat))

      // Longitude
      const rawLng = lines['lon'] || lines['long'] || lines['longitude'] || ''
      if (rawLng) setLng(parseFloat(rawLng))

      // Timezone: JHD uses decimal offset e.g. 5.5
      const rawTz = lines['tz'] || lines['timezone'] || lines['gmtoffset'] || ''
      if (rawLat && rawLng) {
        // Try to derive IANA timezone from coordinates (approximate)
        const lat = parseFloat(rawLat)
        const lng = parseFloat(rawLng || '0')
        // Use offset to make a best-guess timezone name
        if (rawTz) {
          const offset = parseFloat(rawTz)
          // Common IST
          if (Math.abs(offset - 5.5) < 0.1) setTz('Asia/Kolkata')
          else if (Math.abs(offset - 5.75) < 0.1) setTz('Asia/Kathmandu')
          else if (Math.abs(offset - 6) < 0.1) setTz('Asia/Dhaka')
          else if (Math.abs(offset - 5) < 0.1) setTz('Asia/Karachi')
          else if (Math.abs(offset) < 0.1) setTz('UTC')
          else if (Math.abs(offset - (-5)) < 0.1) setTz('America/New_York')
          else if (Math.abs(offset - (-8)) < 0.1) setTz('America/Los_Angeles')
          else if (Math.abs(offset - 1) < 0.1) setTz('Europe/Paris')
          else if (Math.abs(offset - 8) < 0.1) setTz('Asia/Singapore')
          // fallback: keep existing tz
        }
      }

      // Place name
      const rawPlace = lines['place'] || lines['city'] || lines['location'] || ''
      if (rawPlace) setPlace(rawPlace)

    } catch {
      setJhdError('Could not parse JHD file — invalid format')
    }
  }


  function parseSJS(text: string): void {
    setJhdError(null)
    try {
      // SJS is XML — extract attributes from <person ... />
      const match = text.match(/<person([^>]+)>/i)
      if (!match) throw new Error('No <person> tag found')
      const attrs = match[1]
      ...
      const n = attr('name') || attr('fullname')
      if (n) setName(n)

      const rawDate = attr('birthdate') || attr('date')
      if (rawDate) {
        const parts = rawDate.split(/[\/\-\.]+/)
        if (parts.length === 3) {
          let y: string, m2: string, d: string
      ...
      const rawTime = attr('birthtime') || attr('time')
      if (rawTime) {
      ...
      const latStr = attr('lat') || attr('latitude')
      const lngStr = attr('lon') || attr('lng') || attr('longitude')
      if (latStr) setLat(parseFloat(latStr))
      if (lngStr) setLng(parseFloat(lngStr))

      const city = attr('city') || attr('place') || attr('location')
      if (city) setPlace(city)

      const tzStr = attr('tz') || attr('timezone') || attr('gmtoffset')
      if (tzStr) {
        const offset = parseFloat(tzStr)
        if (Math.abs(offset - 5.5) < 0.1) setTz('Asia/Kolkata')
        else if (Math.abs(offset - 5.75) < 0.1) setTz('Asia/Kathmandu')
        else if (Math.abs(offset - 6) < 0.1) setTz('Asia/Dhaka')
      }
      ...
      setJhdError('Could not parse SJS file — invalid format')
    }
  }
  ...
  function handleImportFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (ev) => {
      ...
      if (file.name.endsWith('.sjs') || text.trim().startsWith('<')) {
        parseSJS(text)
      } else {
        parseJHD(text)
      }
    ...
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', width: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
        <input
          ref={jhd_inputRef}
          type="file"
          accept=".jhd,.jyotish,.sjs,.txt"
          onChange={handleImportFile}
          style={{ display: 'none' }}
        />
        <button
          type="button"
          onClick={() => jhd_inputRef.current?.click()}
          style={{
        ...
        >
          📂 Import Chart (.jhd / .sjs)
        </button>
        {jhdError && (
          <span style={{ fontSize: '0.72rem', color: 'var(--rose)', fontFamily: 'var(--font-display)' }}>
            {jhdError}
          </span>
        )}
        <span style={{ fontSize: '0.65rem', color: 'var(--text-muted)', fontFamily: 'var(--font-display)', fontStyle: 'italic' }}>
          Jagannatha Hora or Sri Jyoti Star
        </span>
      </div>
