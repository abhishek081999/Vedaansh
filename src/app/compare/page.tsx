...
function getCompatibility(a: ChartOutput, b: ChartOutput): CompatItem[] {
  ...
  const aM = a.grahas.find(g => g.id === 'Mo')
  const bM = b.grahas.find(g => g.id === 'Mo')
  const aV = a.grahas.find(g => g.id === 'Ve')
  const bV = b.grahas.find(g => g.id === 'Ve')
  const aJ = a.grahas.find(g => g.id === 'Ju')
  const bJ = b.grahas.find(g => g.id === 'Ju')
  const aS = a.grahas.find(g => g.id === 'Su')
  const bS = b.grahas.find(g => g.id === 'Su')
  ...
  if (aA) {
    ...
    const lord7 = a.grahas.find(g => g.id === h7lord)
    if (lord7 && bMoonS) {
          ...
          <div style={{ display:'flex',flexDirection:'column',gap:'1.25rem' }}>
            ...
            {(view === 'ashtakoot' || view === 'all') && (
              <div className="card fade-up" style={{ padding:'2rem', maxWidth: 1000, margin: '0 auto', width: '100%' }}>
                ...
                {(() => {
                  const aMoon = chartA.grahas.find(g => g.id === 'Mo');
                  const bMoon = chartB.grahas.find(g => g.id === 'Mo');
