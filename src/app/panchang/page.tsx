    ...
    <main style={{
      ...
      {data && !loading && (
        <>
          <div style={{
            ...
            {[
              ...
              <div key={label} style={{ textAlign: 'center' }}>
                ...
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: '1rem', fontWeight: 600, color }}>{fmtTime(time)}</div>
              </div>
            ))}
            <div style={{ width: 1, height: 40, background: 'var(--border)' }} />
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '1.4rem' }}>⏱</div>
              <div style={{ fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-muted)', fontFamily: 'var(--font-display)' }}>Day Length</div>
          ...
          <div>
            <div className="label-caps" style={{ marginBottom: '0.75rem' }}>Pañca Aṅga — Five Limbs</div>
            <div className="panchang-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '0.75rem' }}>
              <PanchangCard label="Vara (Weekday)" value={data.vara.name} sub={`${data.vara.sanskrit} · Lord: ${data.vara.lord}`} icon="☀" highlight />
              <div className="fade-up" style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 'var(--r-lg)', padding: '1rem 1.1rem' }}>
                <div style={{ fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-muted)', fontFamily: 'var(--font-display)', marginBottom: '0.3rem' }}>🌙 Tithi</div>
                <div style={{ fontSize: '1.05rem', fontWeight: 600, fontFamily: 'var(--font-display)', color: 'var(--text-primary)' }}>{data.tithi.name}</div>
                <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontFamily: 'var(--font-display)', fontStyle: 'italic', marginBottom: '0.4rem' }}>{data.tithi.paksha === 'shukla' ? 'Śukla Pakṣa' : 'Kṛṣṇa Pakṣa'} · Lord: {data.tithi.lord}</div>
                <TithiProgress percent={data.tithi.percent} paksha={data.tithi.paksha} />
              </div>
              <PanchangCard label="Nakshatra" value={data.nakshatra.name} sub={`Pada ${data.nakshatra.pada} · Lord: ${data.nakshatra.lord}`} icon="⭐" />
              <div className="fade-up" style={{ 
                background: data.yoga.quality === 'auspicious' ? 'rgba(78,205,196,0.06)' : data.yoga.quality === 'inauspicious' ? 'rgba(224,123,142,0.06)' : 'var(--surface-2)',
                border: `1px solid ${data.yoga.quality === 'auspicious' ? 'rgba(78,205,196,0.20)' : data.yoga.quality === 'inauspicious' ? 'rgba(224,123,142,0.20)' : 'var(--border)'}`,
                borderRadius: 'var(--r-lg)', padding: '1rem 1.1rem'
              }}>
                <div style={{ fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-muted)', fontFamily: 'var(--font-display)', marginBottom: '0.3rem' }}>☯ Yoga</div>
                <div style={{ fontSize: '1.05rem', fontWeight: 600, fontFamily: 'var(--font-display)', color: data.yoga.quality === 'auspicious' ? 'var(--teal)' : data.yoga.quality === 'inauspicious' ? 'var(--rose)' : 'var(--text-primary)' }}>{data.yoga.name}</div>
                <div style={{ fontSize: '0.72rem', fontFamily: 'var(--font-display)', fontStyle: 'italic', marginTop: 2, color: data.yoga.quality === 'auspicious' ? 'var(--teal)' : data.yoga.quality === 'inauspicious' ? 'var(--rose)' : 'var(--text-muted)' }}>{data.yoga.quality}</div>
              </div>
              <PanchangCard label="Karaṇa" value={data.karana.name} sub={data.karana.isBhadra ? '⚠ Bhadra — avoid auspicious acts' : data.karana.type} icon="✦" warning={data.karana.isBhadra} />
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <MuhurtaBar label="Rāhu Kālam" start={data.rahuKalam.start} end={data.rahuKalam.end} color="var(--rose)" bg="#e07b8e" />
              <MuhurtaBar label="Gulikā Kālam" start={data.gulikaKalam.start} end={data.gulikaKalam.end} color="var(--rose)" bg="#e07b8e" />
              <MuhurtaBar label="Yamagaṇḍa" start={data.yamaganda.start} end={data.yamaganda.end} color="var(--amber)" bg="#f59e42" />
              {data.abhijitMuhurta && <MuhurtaBar label="Abhijit Muhūrta ✦ Auspicious" start={data.abhijitMuhurta.start} end={data.abhijitMuhurta.end} color="var(--teal)" bg="#4ecdc4" />}
            </div>
          </div>

          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
            <div style={{ flex: 1, minWidth: 200, padding: '0.85rem 1.1rem', background: 'rgba(232,167,48,0.07)', border: '1px solid rgba(232,167,48,0.20)', borderRadius: 'var(--r-md)' }}>
              <div style={{ fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#e8a730', fontFamily: 'var(--font-display)', marginBottom: 4 }}>☀ Sun Nakshatra</div>
              <span style={{ fontFamily: 'var(--font-display)', fontWeight: 600, color: 'var(--text-primary)' }}>{data.sunNakshatra.name}</span>
              <span style={{ marginLeft: 8, fontSize: '0.78rem', color: 'var(--text-muted)', fontStyle: 'italic' }}>Pada {data.sunNakshatra.pada} · Lord {data.sunNakshatra.lord}</span>
            </div>
            <div style={{ flex: 1, minWidth: 200, padding: '0.85rem 1.1rem', background: 'rgba(176,200,224,0.07)', border: '1px solid rgba(176,200,224,0.20)', borderRadius: 'var(--r-md)' }}>
              <div style={{ fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#b0c8e0', fontFamily: 'var(--font-display)', marginBottom: 4 }}>☽ Moon Nakshatra</div>
              <span style={{ fontFamily: 'var(--font-display)', fontWeight: 600, color: 'var(--text-primary)' }}>{data.nakshatra.name}</span>
              <span style={{ marginLeft: 8, fontSize: '0.78rem', color: 'var(--text-muted)', fontStyle: 'italic' }}>Pada {data.nakshatra.pada} · Lord {data.nakshatra.lord}</span>
            </div>
          </div>

          <div>
            <button onClick={() => setHoraOpen(o => !o)} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-display)', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: horaOpen ? '0.75rem' : 0, padding: 0 }}>
            ...
            {horaOpen && (
              <div style={{ background: 'var(--surface-1)', border: '1px solid var(--border)', borderRadius: 'var(--r-lg)', overflow: 'hidden' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', borderBottom: '1px solid var(--border-soft)' }}>
                  {['Day Horās', 'Night Horās'].map(label => (<div key={label} style={{ padding: '0.5rem 0.85rem', fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.09em', textTransform: 'uppercase', color: 'var(--text-muted)', fontFamily: 'var(--font-display)', borderRight: label === 'Day Horās' ? '1px solid var(--border-soft)' : 'none' }}>{label}</div>))}
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
                  <div style={{ borderRight: '1px solid var(--border-soft)' }}>{data.horaTable.filter(h => h.isDaytime).map((h, i) => (<HoraRow key={i} hora={h} />))}</div>
                  <div>{data.horaTable.filter(h => !h.isDaytime).map((h, i) => (<HoraRow key={i} hora={h} />))}</div>
                </div>
              </div>
            )}
          </div>
